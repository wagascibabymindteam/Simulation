...
void PMAnaSummary::AddIngridHitTrk(IngridHitSummary* hit, int trk)
{

  if (trk < INGRIDRECON_MAXTRACKS) {
    if (nhitTs[trk] < INGRIDHIT_MAXHITS) {
      fIngridHitTrk[nhitTs[trk]][trk] = TRef((IngridHitSummary*) hit);
      fIngridHit[nhits] = TRef((IngridHitSummary*) hit);
      ++nhitTs[trk];
      ++nhits;
    }
...
IngridHitSummary* PMAnaSummary::GetIngridHitTrk(int i, int j) const
{
