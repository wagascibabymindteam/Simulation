...
//////////////////////////////////////////////////////////////
  ...
  moduleLV[16] = new G4LogicalVolume(Pmodule_box, Air, "module_log");
  ...
  for(int i=0;i<18;i++){
    ...
    if (i==0){
      for(int j=0;j<24;j++){ 
        ...
        G4double y_scinti =   scibar_xy_start + 2*j*scibar_y;
	new G4PVPlacement(0,G4ThreeVector(0.,y_scinti,z),hscint_intLV,name,moduleLV[16],false,j+k*264); // in Module
        sprintf(name,"x%d",j+k*264);
        G4double x_scinti =  scibar_xy_start + 2*j*scibar_y;
	new G4PVPlacement(0,G4ThreeVector(x_scinti,0.,z+dist),vscint_intLV,name,moduleLV[16],false,j+k*264); // in Module       
      }
    }
    else{
      //---------- 17 TKP
      ...
      for(int j=0;j<8;j++){ 
        char name[10]; 
        sprintf(name,"y%d",j+32*(i-1)+k*264+24);
        G4double y_scinti = scibar_xy_start + 2*j*scibar_y;       
	new G4PVPlacement(0,G4ThreeVector(0,y_scinti,z),hscint_intLV,name,moduleLV[16],false,j+32*(i-1)+k*264+24); // horizontal bars
        sprintf(name,"x%d",j+32*(i-1)+k*264+24);
        G4double x_scinti = scibar_xy_start + 2*j*scibar_y; 
	new G4PVPlacement(0,G4ThreeVector(x_scinti,0.,z+dist),vscint_intLV,name,moduleLV[16],false,j+32*(i-1)+k*264+24); // vertical bars
      }
	...
	new G4PVPlacement(0,G4ThreeVector(0,y_scinti,z),hscint2_intLV,name,moduleLV[16],false,j+32*(i-1)+k*264+24); // horizontal bars
        sprintf(name,"x%d",j+32*(i-1)+k*264+24);
        G4double x_scinti = scibar_xy_start + 15*scibar_y +(2*(j-8)+1)*scibar2_y; 
        new G4PVPlacement(0,G4ThreeVector(x_scinti,0.,z+dist),vscint2_intLV,name,moduleLV[16],false,j+32*(i-1)+k*264+24); // vertical bars
      }
